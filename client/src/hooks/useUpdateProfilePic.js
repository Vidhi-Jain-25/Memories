...
const url = process.env.REACT_APP_ENDPOINT ||  "http://localhost:5000";
...
const useUpdateProfilePic = ({ profile_pic, cover_pic, history }) => {
  ...
  const saveProfilePic = async (profile_url) => {
    ...
    try {
      ...
      let i=accounts.findIndex((acc)=>acc.name===response.data.user.name)
      console.log(i)
      console.log(accounts[i])
      accounts[i].profile_pic= response.data.user.profile_pic;
      // accounts.push(me.data.user);
