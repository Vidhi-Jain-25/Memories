...
exports.sendFriendRequest = async (req, res) => {
  try {
    ...
    if (friendRequest) {
      return res.status(400).json({ error: 'Friend Request already sended' })
    }
...
exports.updateCoverPic = async (req, res) => {
  ...
  try {
    ...
    userData.friends = friends
    res.status(200).json({ message: 'Cover pic updated!', user: userData })
  } 
  catch (err) {
    console.log(err)
    return res.status(500).json({ error: "Something went wrong" })
...
exports.updateProfile = async (req, res) => {
  try {
    ...
    await user.save()
    res.status(200).json({data:req.userId, message: 'Profile Updated Successfully' })
  } 
  catch (err) {
    console.log(err)
    return res.status(500).json({ error: "Something went wrong" })
...
exports.deleteFriend = async (req, res) => {
  try { 
    const frend= await User.findById(req.params.id).populate('friends');
    const mee= await User.findById(req.userId).populate('friends');
  
    const gg=mee.friends.filter((fre)=>{
      return req.params.id!==fre.id
    })
    mee.friends=gg;
    await mee.save();
    
    const g=frend.friends.filter((fre)=>{
      return req.userId!==fre.id
    })  
 
    frend.friends=g; 
    await frend.save();  
    // console.log(req.params.requestId)
    res.status(200).json({ message: 'Friend deleted', data: mee , friend:frend});
    const user= frend
